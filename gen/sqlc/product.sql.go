...
SELECT id, $6, 0
FROM new_product
RETURNING product_id, current_stock, sold
`
...
WITH filtered_product AS (
    SELECT p.id, p.product_model_id, p.additional_price, p.is_active, p.can_combine, p.metadata, p.date_created, pt.current_stock, pt.sold
    FROM product.base p
    LEFT JOIN product.tracking pt ON p.id = pt.product_id
    WHERE p.id = $1
),
...
SELECT 
    p.id, p.product_model_id, p.additional_price, p.is_active, p.can_combine, p.metadata, p.date_created, p.current_stock, p.sold,
...
WITH filtered_product AS (
    ...
    WHERE (
        ...
        (pt.current_stock >= $5 OR $5 IS NULL) AND
        (pt.current_stock <= $6 OR $6 IS NULL) AND
        (pt.sold >= $7 OR $7 IS NULL) AND
        (pt.sold <= $8 OR $8 IS NULL) AND
        (p.additional_price >= $9 OR $9 IS NULL) AND
        (p.additional_price <= $10 OR $10 IS NULL) AND
