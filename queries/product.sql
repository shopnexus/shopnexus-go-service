...
WITH filtered_product AS (
    SELECT p.*, pt.current_stock, pt.sold
    FROM product.base p
...
WITH filtered_product AS (
    ...
    WHERE (
        ...
        (pt.sold >= sqlc.narg('sold_from') OR sqlc.narg('sold_from') IS NULL) AND
        (pt.sold <= sqlc.narg('sold_to') OR sqlc.narg('sold_to') IS NULL) AND
        (p.additional_price >= sqlc.narg('additional_price_from') OR sqlc.narg('additional_price_from') IS NULL) AND
        (p.additional_price <= sqlc.narg('additional_price_to') OR sqlc.narg('additional_price_to') IS NULL) AND
...
INSERT INTO product.tracking (product_id, current_stock, sold)
SELECT id, $6, 0
FROM new_product
RETURNING *;
...
UPDATE product.tracking
SET
    current_stock = current_stock - sqlc.arg('amount'),
    sold = sold + sqlc.arg('amount')
WHERE
    (product_id = ANY(sqlc.arg('ids')::bigint[])) AND 
    (current_stock - sqlc.arg('amount') >= 0);
