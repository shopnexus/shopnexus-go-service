...
WITH filtered_comment AS (
    ...
    WHERE
        ...
        (c.downvote <= sqlc.narg('downvote_to') OR sqlc.narg('downvote_to') IS NULL) AND
        (c.score >= sqlc.narg('score_from') OR sqlc.narg('score_from') IS NULL) AND
        (c.score <= sqlc.narg('score_to') OR sqlc.narg('score_to') IS NULL) AND
        (c.date_created >= sqlc.narg('created_at_from') OR sqlc.narg('created_at_from') IS NULL) AND
        (c.date_created <= sqlc.narg('created_at_to') OR sqlc.narg('created_at_to') IS NULL)
),
filtered_resources AS (
    SELECT 
        res.owner_id,
