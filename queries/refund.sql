...
SELECT 
  r.*,
  COALESCE(array_agg(DISTINCT res.key) FILTER (WHERE res.key IS NOT NULL), '{}')::text[] as resources
FROM payment.refund r
...
SELECT r.id, COALESCE(array_agg(DISTINCT res.key) FILTER (WHERE res.key IS NOT NULL), '{}')::text[] as resources
