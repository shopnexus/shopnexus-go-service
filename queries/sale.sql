...
WHERE
    (sqlc.narg('type')::text IS NULL OR s.type = sqlc.narg('type')) AND
    (sqlc.narg('item_id')::bigint IS NULL OR s.item_id = sqlc.narg('item_id')) AND
    (sqlc.narg('date_started_from')::timestamptz IS NULL OR s.date_started >= sqlc.narg('date_started_from')) AND
    (sqlc.narg('date_started_to')::timestamptz IS NULL OR s.date_started <= sqlc.narg('date_started_to')) AND
    (sqlc.narg('date_ended_from')::timestamptz IS NULL OR s.date_ended >= sqlc.narg('date_ended_from')) AND
    (sqlc.narg('date_ended_to')::timestamptz IS NULL OR s.date_ended <= sqlc.narg('date_ended_to')) AND
    (sqlc.narg('is_active')::boolean IS NULL OR s.is_active = sqlc.narg('is_active'));
